...
function runTime(startOn, endOn) {
    if (endOn !== undefined) {
        ...
        return moment.utc(moment.duration(diff_seconds, "seconds").asMilliseconds()).format("H:mm:ss")
    } else {
        return "Running"
    }
}
    ...
    },
    {
        ...
        cell: (item) => {
            if (item.state === "SUCCEEDED") {
                ...
                return <StatusIndicator type="info">{item.state}</StatusIndicator>;
            } else if (item.state === "FAILED") {
                return <StatusIndicator type="error">{item.state}</StatusIndicator>;
            }
        },
    },
];
